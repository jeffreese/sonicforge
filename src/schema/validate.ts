...
export function validate(data: unknown): SonicForgeComposition {
  ...
  if (!meta || typeof meta !== "object") {
    ...
    if (typeof meta.bpm !== "number" || meta.bpm <= 0) errors.push("metadata.bpm must be a positive number");
  ...
  if (!Array.isArray(instruments) || instruments.length === 0) {
    ...
    for (const inst of instruments) {
      ...
      ids.add(inst.id);
      if (typeof inst.sample !== "string") errors.push(`instrument "${inst.id}": sample must be a string`);
      const validCategories = ["melodic", "bass", "pad", "drums", "fx"];
      if (!validCategories.includes(inst.category)) {
        errors.push(`instrument "${inst.id}": category must be one of ${validCategories.join(", ")}`);
      }
    }
  }
      ...
      if (!Array.isArray(section.tracks)) {
        ...
        for (const track of section.tracks) {
          ...
          if (!Array.isArray(track.notes)) {
            errors.push(`section "${section.name}", track "${track.instrumentId}": notes must be an array`);
          }
        }
      }
    }
