...
export class Timeline {
  ...
  private resize(): void {
    ...
    const height = this.HEADER_HEIGHT + Math.max(this.instruments.length, 1) * this.ROW_HEIGHT + 4
  ...
  private draw(): void {
    ...
    for (let i = 0; i < this.sectionOffsets.length; i++) {
      ...
      ctx.fillStyle = '#ddd'
      ctx.fillText(so.section.name, x + sectionWidth / 2, this.HEADER_HEIGHT - 9)
    ...
    for (let r = 0; r < this.instruments.length; r++) {
      ...
      // Label
      ctx.fillStyle = '#aaa'
      ctx.fillText(this.instruments[r].name, gridLeft - 8, y + this.ROW_HEIGHT / 2 + 4)
