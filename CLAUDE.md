...
- **Engine** (`src/engine/`) — TypeScript playback engine using Tone.js + SoundFont CDN samples. Validates JSON, loads samples, schedules notes, manages transport.
...
- Samples loaded from `gleitz/midi-js-soundfonts` CDN (FluidR3_GM, OGG format)
...
src/engine/     — Playback engine (Engine, Transport, TrackPlayer, InstrumentLoader)
